...
# Set configurations for Twython API
...
f= open("../data/rawData.json","a+")

class MyStreamer(TwythonStreamer):
  # Print the current datetime value when streaming data
  def on_success(self, data):
    if ('place' in data and data['place'] is not None) or ('location' in data['user'] and data['user']['location'] is not None):
      print(datetime.now())
      f.write(json.dumps(data) + ',\n')

  # Print the error status code if streaming fails
...
for i in range(10): # Filter for English tweets mentioning any of the brands
